...
namespace Tests
{
	public class PaymentCardSecretAsyncTests
	{
		...
		public async Task GetWithInvalidKeyAsyncTest()
		{
			...
			// Assert
			Assert.ThrowsAsync<ArgumentNullException>(async () => await paymentCardSecret.GetTitleAsync(null, securityAsyncFunctions));
			Assert.ThrowsAsync<ArgumentException>(async () => await paymentCardSecret.GetTitleAsync(derivedKeyInvalid, securityAsyncFunctions));
		}
		...
		public async Task CheckIfChecksumMatchesContentAsyncTest()
		{
			...
			PaymentCard paymentCard = await PaymentCard.CreatePaymentCardAsync("Bank of Dragon III", "Coolah Dragon", "Debit", "0000000000001234", "111", "11/20", "05/33", "Super cool card I have here", securityAsyncFunctions);

			PaymentCardSecret paymentCardSecret = await PaymentCardSecret.CreatePaymentCardSecretAsync(paymentCard, "does not matter", skaAES_CTR, derivedKey, securityAsyncFunctions);

			// Act
			bool shouldBeTrue = await paymentCardSecret.CheckIfChecksumMatchesContentAsync(securityAsyncFunctions);
			paymentCardSecret.checksum = paymentCardSecret.checksum.Remove(0, 1);
			bool shouldBeFalse = await paymentCardSecret.CheckIfChecksumMatchesContentAsync(securityAsyncFunctions);
