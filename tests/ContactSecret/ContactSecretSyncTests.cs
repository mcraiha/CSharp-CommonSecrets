...
namespace Tests
{
	public class ContactSecretSyncTests
	{
		...
		public void GetWithInvalidKeyTest()
		{
			...
			// Assert
			Assert.Throws<ArgumentNullException>(() => cs.GetFirstName(null));
			Assert.Throws<ArgumentException>(() => cs.GetFirstName(derivedKeyInvalid));
		}
		...
		public void GetSingleEntriesTest()
		{
			...
			Assert.AreEqual(String.Join(Contact.separatorChar, emails), cs.GetEmails(derivedKey));
			Assert.AreEqual(String.Join(Contact.separatorChar, emailDescriptions), cs.GetEmailDescriptions(derivedKey));
			Assert.AreEqual(String.Join(Contact.separatorChar, phoneNumbers), cs.GetPhoneNumbers(derivedKey));
			Assert.AreEqual(String.Join(Contact.separatorChar, phoneNumberDescriptions), cs.GetPhoneNumberDescriptions(derivedKey));
			CollectionAssert.AreEqual(emails, cs.GetEmailsArray(derivedKey));
			CollectionAssert.AreEqual(emailDescriptions, cs.GetEmailDescriptionsArray(derivedKey));
			CollectionAssert.AreEqual(phoneNumbers, cs.GetPhoneNumbersArray(derivedKey));
			CollectionAssert.AreEqual(phoneNumberDescriptions, cs.GetPhoneNumberDescriptionsArray(derivedKey));
		...
		public void CheckIfChecksumMatchesContentTest()
		{
			...
			// Act
			bool shouldBeTrue = cs1.CheckIfChecksumMatchesContent();
			cs1.checksum = cs1.checksum.Remove(0, 1);
			bool shouldBeFalse = cs1.CheckIfChecksumMatchesContent();
