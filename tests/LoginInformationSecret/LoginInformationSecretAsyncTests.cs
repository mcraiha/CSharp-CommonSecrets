...
namespace Tests
{
	public class LoginInformationSecretAsyncTests
	{
		...
		public async Task GetWithInvalidKeyAsyncTest()
		{
			...
			// Assert
			Assert.ThrowsAsync<ArgumentNullException>(async () => await loginInformationSecret.GetTitleAsync(null, securityAsyncFunctions));
			Assert.ThrowsAsync<ArgumentException>(async () => await loginInformationSecret.GetTitleAsync(derivedKeyInvalid, securityAsyncFunctions));
		}
		...
		public async Task CheckIfChecksumMatchesContentAsyncTest()
		{
			...
			// Act
			bool shouldBeTrue = await loginInformationSecret1.CheckIfChecksumMatchesContentAsync(securityAsyncFunctions);
			loginInformationSecret1.checksum = loginInformationSecret1.checksum.Remove(0, 1);
			bool shouldBeFalse = await loginInformationSecret1.CheckIfChecksumMatchesContentAsync(securityAsyncFunctions);
